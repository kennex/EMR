		...
		if (index < tourItems.length && index >= 0) {
			...
			if (tourItem.displayType == "modal") {
				...
				$('#modal-continue').live('click', function () {
					...
					setTimeout(function () {
						displayTourItem(currentTourIndex)
					}, 1);
				});
				...
				var tempContent = tourItem.content
						+
						'<br/><div><p style="font-size: 10px;">Press Tab/Enter or Continue Button when done</p><button class="btn btn-success" id="popover-continue-' +
						index + '">' + buttonText + '</button></div>';
				$("#" + tourItem.fieldId).popover(
						{
							content: tempContent,
							trigger: 'manual',
							placement: mobileMode && tourItem.mobilePlacement != undefined ?
									tourItem.mobilePlacement :
									tourItem.placement,
							title: tourItem.title
						}
				);
		...
		for (var i = 0; i < tourItems.length; i++) {
			...
			if (tourItem.displayType == "popover") {
				...
				if (i == 0) {
					...
					$('#vtour-backdrop').on('click', function (e) {
						vTour.hideTourGuide();
					});
